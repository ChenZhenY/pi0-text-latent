...
def eval_libero(args: Args) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        scence_prompt = "pick up the alphabet soup and place it in the basket"
        description_prompt = "pick up the bbq sauce and place it in the basket"
        ...
        for swap_step in rollout_task_swap_step:
            if args.save_video:
            ...
            for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
                ...
                while t < max_steps + args.num_steps_wait:
                    try:
                        ...
                        # Determine which prompt is currently active for this timestep
                        if t >= swap_step + args.num_steps_wait:
                            current_prompt = str(task_description_change)
                        else:
                            current_prompt = str(task_description)

                        # Add prompt text to the agentview image for visualization
                        ...
                        thickness = 2
                        margin = 10
                        # Split prompt into multiple lines if too long
                        max_line_length = 40
                        prompt_lines = [current_prompt[i:i+max_line_length] for i in range(0, len(current_prompt), max_line_length)]
                        y0 = margin + 30
                        for i, line in enumerate(prompt_lines):
                            y = y0 + i * 30
                            cv2.putText(annotated_img, line, (margin, y), font, font_scale, font_color, thickness, cv2.LINE_AA)

                        if args.render:
                            cv2.imshow("agentview", cv2.cvtColor(annotated_img, cv2.COLOR_BGR2RGB))
                            cv2.imshow("robot0_eye_in_hand", cv2.cvtColor(original_wrist_img, cv2.COLOR_BGR2RGB))
                            cv2.waitKey(1)
                        # Save preprocessed image for replay video (with prompt annotation)
                        ...
                        if not action_plan:
                            ...
                            # Use task_description_change at the specified swap_step
                            if t >= swap_step + args.num_steps_wait:
                                element["prompt"] = str(task_description_change)
                                if "has_logged_swap" not in locals():
                                    has_logged_swap = False
                                if not has_logged_swap:
                                    logging.info(f"Switched to task_description_change at step {t} and following: {task_description_change}")
                                    has_logged_swap = True
                ...
                if args.save_video:
                    imageio.mimwrite(
                        pathlib.Path(args.video_out_path) / f"{task_segment}/{swap_step_dir}_{swap_prompt}/{suffix}_{episode_idx}.mp4",
                        [np.asarray(x) for x in replay_images],
                        fps=30,  # the same as openvla
                    )
                    imageio.mimwrite(
                        pathlib.Path(args.video_out_path) / f"{task_segment}/{swap_step_dir}_{swap_prompt}/{suffix}_wrist_{episode_idx}.mp4",
                        [np.asarray(x) for x in replay_wrist_images],
                        fps=30)  # the same as openvla

                if args.draw_traj:
                    dir = pathlib.Path(args.video_out_path) / f"{task_segment}/{swap_step_dir}_{swap_prompt}/{suffix}_{episode_idx}"
